...
仓库提交了两个遗留校准文件，用于开发和历史复现，但 Python wheel 不包含它们。
它们的生成溯源不完整。运维人员必须为实际服务的精确模型 revision 生成或安全
分发具备完整溯源的产物，也不能从本仓库许可证推定原始模型/数据集的再分发权。

## 已提交的遗留产物

仓库包含两个 Qwen2.5-Coder-14B-Instruct 产物：
